...
export default class HiringMiddleware {
  @HandleException()
  public static async addHiringDetailsValidator(request: Request, response: Response, next: NextFunction) {
    ...
    const schema = Joi.object<IHiring>().keys({
      client_name: Joi.string().min(2).pattern(Patterns.common.name).required().messages(ValidationMessages.hiring.client_name),
      client_email: Joi.string().min(5).pattern(Patterns.common.email).required().messages(ValidationMessages.hiring.client_email),
      client_project_name: Joi.string().min(2).max(255).required().messages(ValidationMessages.hiring.client_project_name),
      tenure: Joi.number().min(1).max((Number.MAX_SAFE_INTEGER - 1)).optional().messages(ValidationMessages.hiring.tenure),
      hiring_type: Joi.string().valid(HiringType.FULL_TIME, HiringType.PART_TIME).required().messages(ValidationMessages.hiring.hiring_type),
      ...
      project_desc: Joi.string().min(10).max(255).required().messages(ValidationMessages.hiring.project_desc),
    ...
    if (payload.hiring_type === HiringType.FULL_TIME && payload.tenure) {
      logger.info({ message: `Failed to validate hiring form's payload due to either hiring type (${payload.hiring_type}) or tenure (${payload.tenure}).` });
  ...
  @HandleException()
  public static async validateHiringFormCaptcha(request: Request, response: Response, next: NextFunction) {
    const reply = new ApiResponse();
    const logger = init();
    ...
    if (!stringifiedPayload) {
      logger.info({ message: `No captcha id (${captchaId}) was found. Either captcha has expired or captcha id is invalid.` });
