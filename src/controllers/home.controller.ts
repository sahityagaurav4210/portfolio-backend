...
class HomeController {
  ...
  public static async captcha(request: Request, response: Response): Promise<Response> {
    ...
    logger.info({
      message: `Captcha - ${captcha} has been generated at ${new Date().toISOString()} by ${
        request.ip || '0.0.0.0'
      }`,
    });
  ...
  public static async refreshCaptcha(request: Request, response: Response): Promise<Response> {
    ...
    logger.info({
      message: `Captcha - ${captcha} with captcha id - ${captchaId} has been deleted successfully at ${new Date().toISOString()} by ${
        request.ip || '0.0.0.0'
      }`,
    });
    ...
    logger.info({
      message: `Captcha - ${captcha} has been re-generated at ${new Date().toISOString()} by ${
        request.ip || '0.0.0.0'
      }`,
    });
  ...
  @HandleException()
  public static async captchaValidate(request: Request, response: Response): Promise<void> {
    const reply = new ApiResponse();
    ...
    if (decryptedCaptcha !== captcha) {
      ...
      logger.info({
        message: `Provided captcha does not match with the one which has been generated by the system, please check the typos in captcha provided.`,
      });
      response.status(HTTP_STATUS_CODES.UNAUTHORISED).json(reply);
      return;
    }
  ...
  @HandleException()
  public static async addUserHomeSection(request: CustomReq, response: Response): Promise<void> {
    const reply = new ApiResponse();
    const logger = init();
    ...
    response.status(HTTP_STATUS_CODES.CREATED).json(reply);
  }

  @HandleException()
  public static async getUserHomeSection(
    request: CustomReq,
    response: Response
  ): Promise<Response> {
    const reply = new ApiResponse();
