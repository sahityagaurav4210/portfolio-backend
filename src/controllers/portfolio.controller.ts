...
class PortfolioController {
  ...
  public static async create(request: CustomReq, response: Response): Promise<Response> {
    ...
    [portfolio] = await Promise.allSettled([Portfolio.create(request.body), Events.create({ eventName: EventNames.PORTFOLIO_CREATED, firedBy: authenticatedUser._id })]);
  ...
  public static async list(request: CustomReq, response: Response): Promise<Response> {
    ...
    [portfolios] = await Promise.allSettled([Portfolio.aggregate(Queries.listPortfolio()), Events.create({ eventName: EventNames.ALL_PORTFOLIO_FETCHED, firedBy: authenticatedUser._id })]);
  ...
  public static async get(request: CustomReq, response: Response): Promise<Response> {
    ...
    const { authenticatedUser } = request;
    const eventName = EventNames.SINGLE_PORTFOLIO_FETCHED.replace(":portfolio_user", portfolio_user);
    let portfolio, code: number;

    [portfolio] = await Promise.allSettled([Portfolio.aggregate([
      {
        $match: {
  ...
  public static async edit(request: CustomReq, response: Response): Promise<Response> {
    ...
    [updatedRecord] = await Promise.allSettled([Portfolio.findByIdAndUpdate(portfolioId, request.body, modelUpdateObject()), Events.create({ eventName: EventNames.PORTFOLIO_EDITED, firedBy: authenticatedUser._id })]);
